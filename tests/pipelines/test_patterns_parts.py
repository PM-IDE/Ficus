...
def test_discover_activities_from_primitive_tandem_arrays():
    ...
    assert list(map(lambda x: x.repeat_set, activities(result))) == [SubArrayWithTraceIndex(first_pos=2, length=3, trace_index=0)]
...
def test_discover_activities_from_maximal_tandem_arrays():
    ...
    assert result.has_value(activities_key)
    assert list(map(lambda x: x.repeat_set, activities(result))) == [SubArrayWithTraceIndex(first_pos=0, length=3, trace_index=0)]
    assert list(map(str, activities(result))) == ['[98, 99, 100]']
