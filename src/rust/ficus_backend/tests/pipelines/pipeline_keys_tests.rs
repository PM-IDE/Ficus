...
fn test_event_log_all_concrete_keys() {
    execute_test(|keys, _| {
        ...
        assert!(keys.find_concrete_key::<usize>(ContextKeys::EVENTS_COUNT).is_some());
...
fn test_equivalence_of_keys() {
    execute_test(|keys, _| {
        ...
        assert!(keys.find_key(ContextKeys::EVENTS_COUNT).unwrap().key().id() == keys.find_concrete_key::<usize>(ContextKeys::EVENTS_COUNT).unwrap().key().id());
    })
}
