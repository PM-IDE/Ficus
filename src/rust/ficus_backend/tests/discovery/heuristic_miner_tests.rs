use ficus_backend::{event_log::simple::simple_event_log::SimpleEventLog, features::discovery::{heuristic::heuristic_miner::discover_petri_net_heuristic, petri_net::pnml_serialization::serialize_to_pnml}};
...
fn execute_heuristic_miner_discovery_test(test_name: &str, log_creator: impl Fn() -> SimpleEventLog) {
    execute_test_with_gold(get_serialized_petri_nets_gold_path(test_name), || {
        let log = log_creator();
        serialize_to_pnml(&discover_petri_net_heuristic(&log, 0.2, 1, 1.0, 0.1, 0.5), true).ok().unwrap()
    })
}
