...
pub fn calculate_default_dfg_entropy<TLog>(log: &TLog) -> HashMap<String, f32> where TLog: EventLog {
    let calculator = |first: &String, second: &String, log_info: &EventLogInfo| {
    ...
    calculate_dfg_entropy(log, calculator)
}

fn calculate_dfg_entropy<TLog, TDfrEntropyCalculator>(
    log: &TLog,
    calculator: TDfrEntropyCalculator,
) -> HashMap<String, f32>
where 
    TLog: EventLog,
    TDfrEntropyCalculator: Fn(&String, &String, &EventLogInfo) -> f32,
{
    ...
    for event_name in events_names {
        let dfr_vector: Vec<f32> = dfr_events_names
            .iter()
            .map(|current_name| calculator(event_name, current_name, &log_info))
            .collect();

        let dpr_vector: Vec<f32> = dpr_events_names
            .iter()
            .map(|current_name| calculator(current_name, event_name, &log_info))
            .collect();
